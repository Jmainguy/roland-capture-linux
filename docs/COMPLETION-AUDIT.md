...
| Internal/external clock source and lock state | verified-live, prototype only | attached QUAD-CAPTURE COAXIAL OUT provided an exact 48 kHz source; twenty-sample sequences verified internal → external locked → AUTO fallback to internal → external relock. `src/usb_telemetry.rs`; `scripts/verify-clock-state.sh`; `docs/EXTERNAL-CLOCK-TEST.md`. | production exposure requires a later device-specific kernel telemetry interface; it is intentionally outside the initial upstream series |
